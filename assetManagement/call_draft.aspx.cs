...
{
    public partial class call_draft : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow item in grid_display.Rows)
            {
                ...
                cmdd.CommandText = "select draftDate from ast_call where call_id='" + call_id + "'";
                conn_asset.Open();
                OdbcDataReader dr1 = cmdd.ExecuteReader();
                
                DateTime cdat=DateTime.Now;
                while (dr1.Read())
                {
                    cdat = Convert.ToDateTime(dr1["draftDate"]);
               }
                conn_asset.Close();
                
                TextBox draftDate = item.FindControl("txt_date") as TextBox;
                draftDate.Text = cdat.ToString("yyyy-MM-dd");
        ...
        private void BindData()
        {
            ...
            cmd.CommandText = "select c.call_id,c.p_no,e.name,d.deptName,e.location,e.contact_no,c.openingDate,c.userDescription,c.draftDate from ast_call as c inner join ast_empMaster as e on c.p_no=e.p_no inner join ast_deptMaster d on d.deptCode=e.deptCode where callStat='d'";
            conn_asset.Open();
