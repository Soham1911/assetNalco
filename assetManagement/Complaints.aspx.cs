...
=======
                ...
                OdbcCommand cmdf = conn_asset.CreateCommand();
                if(chk == "SOFTWARE")
                    cmdf.CommandText = "select Problem from error_selection where (Type ='SOFTWARE' or Type = 'BOTH') and astType = '" + type.Trim() + "'";
                else
                    cmdf.CommandText = "select Problem from error_selection where (Type ='HARDWARE' or Type = 'BOTH') and astType = '" + type.Trim() + "'";
                OdbcDataAdapter df = new OdbcDataAdapter(cmdf);
                DataTable dt1 = new DataTable();
                df.Fill(dt1);
...
>>>>>>> bd4e0da101a2d4a34eb05525dbed44639ff24e6b
        ...
        protected void btn_reg_Click(object sender, EventArgs e)
        {
            ...
            cmdd.CommandText = "select * from ast_call where astCode='" + astCode + "' and callStat = 'o' and openingDate = '" + date.Trim() +"' ";
            conn_asset.Open();
            ...
            if (flag == 0)
            {
                ...
                cmdc.CommandText = "insert into ast_call( p_no, astCode, category, userDescription, systemDescription, openingDate, type, callStat, openingIP) values ( '" + p_no.Trim() + "', '" + astCode + "','" + type.Trim() + "', '" + txt_desc.Text.Trim() + "','" + Drp_2.DataValueField.Trim() + "' , '" + date.Trim() + " ','" + rdbtn_type.DataValueField.Trim() + "','o', '" + myIP.Trim() + "') ";
                conn_asset.Open();
